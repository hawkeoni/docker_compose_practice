...
gen = pd.read_csv("data/training.1600000.processed.noemoticon.csv", encoding="ISO-8859-1", header=None)
gen.columns = ["target", "id", "date", "flag", "user", "text"]
pol["class"] = 1
gen["class"] = 0
p_clean = lambda x: x.replace("#politics", "").replace("#POLITICS", "") if random() < 0.8 else x
pol.text = pol.text.apply(p_clean)
tables_dataset = (pol[["text", "class"]].iloc[:160_000], gen[["text", "class"]].iloc[:200_000])
tables_generation = (pol[["text", "class"]].iloc[160_000:], gen[["text", "class"]].iloc[200_000:400_000])
